...
### Descripción
Debe explicar breve y sencillamente la funcionalidad, escrita desde la perspectiva de la persona que desea esa nueva capacidad.
...
### Modelo de Datos
Listado de atributos con sus tipos y validaciones.
* Nombres en **negrita**, camelCase y empezando en minúscula.
* Tipo de dato entre paréntesis después del nombre. Usar tipos Java.
* Si es una paramétrica, usar `ParametricValue`.
* Si es booleano o Si/No, usar `boolean`.
* Añadir `*` si es obligatorio.
* Validaciones de integridad (dentro del agregado): obligatoriedad, valores por defecto, rangos, regex para formatos (email, teléfono, etc.).
* Atributos autocalculados: añadir `(autocalculado)` e indicar la fórmula.
* Si la HU es para crear el modelo, incluir todos los atributos. Si no, solo los cambios.

### Contratos
Definición de cambios en APIs (REST, GraphQL, etc.).
* Para REST, incluir: URL, Query parameters, Body (mismo formato que modelo) y Response.
...
* **Caso de uso**: nombre del caso de uso con:
    ...
    * Validaciones de negocio (ej: no modificar si está pagada).
